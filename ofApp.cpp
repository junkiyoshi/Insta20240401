...
void ofApp::draw() {
	...
	for (int i = 0; i < this->box_list.size(); i++) {
		...
		for (int k = 0; k < 400; k++) {
			...
			ofSetColor(color, ofMap(k, 0, 400, 32, 255));

			auto vertex = glm::vec3(
				ofMap(ofNoise(noise_seed.x, k * 0.005 + ofGetFrameNum() * 0.085), 0, 1, -radius * 0.85, radius * 0.85),
				ofMap(ofNoise(noise_seed.y, k * 0.005 + ofGetFrameNum() * 0.085), 0, 1, -radius * 0.85, radius * 0.85),
				ofMap(ofNoise(noise_seed.z, k * 0.005 + ofGetFrameNum() * 0.085), 0, 1, -radius * 0.85, radius * 0.85)
			);

			auto next = glm::vec3(
				ofMap(ofNoise(noise_seed.x, (k + 1) * 0.005 + ofGetFrameNum() * 0.085), 0, 1, -radius * 0.85, radius * 0.85),
				ofMap(ofNoise(noise_seed.y, (k + 1) * 0.005 + ofGetFrameNum() * 0.085), 0, 1, -radius * 0.85, radius * 0.85),
				ofMap(ofNoise(noise_seed.z, (k + 1) * 0.005 + ofGetFrameNum() * 0.085), 0, 1, -radius * 0.85, radius * 0.85)
			);
